...
def test_vertex_short_model_uses_project_and_location(monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        "candidates": [
            {
                ...
                "logprobsResult": {
                    "chosenCandidates": [{"token": "YES", "tokenId": 42, "logProbability": -0.1}],
                    "topCandidates": [{"candidates": [{"token": "YES", "tokenId": 42, "logProbability": -0.1}]}],
                },
            }
        ]
    }
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        assert "projects/p/locations/l/publishers/google/models/gemini-2.0-flash-001" in str(request.url)
        return httpx.Response(200, json=fixture)
