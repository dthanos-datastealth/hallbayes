    ...
    for attempt in range(retries + 1):
        try:
            ...
            if resp.status_code >= 400:
                raise RuntimeError(f"Vertex generateContent failed ({resp.status_code}): {resp.text}")
            ...
            if include_logprobs:
                ...
                if lr is None:
                    raise RuntimeError("Vertex response missing logprobsResult (logprobs were requested)")
